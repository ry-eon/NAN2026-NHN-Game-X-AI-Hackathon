...
describe('Greedy 봇', () => {
  it('stage-001을 헤드리스로 클리어한다 — W1 완료 기준의 봇 절반', () => {
    const result = runHeadless(STAGE_001, UNIT_DEFS, ENEMY_DEFS, createGreedyPolicy())
  ...
  it('봇의 액션 로그를 리플레이하면 동일한 최종 상태가 재현된다', () => {
    ...
    const play = runHeadless(STAGE_001, UNIT_DEFS, ENEMY_DEFS, createGreedyPolicy())
    const replay = runReplay(STAGE_001, UNIT_DEFS, ENEMY_DEFS, play.actionLog)
  ...
  it('같은 스테이지·시드에서 봇 실행은 결정론적이다', () => {
    const a = runHeadless(STAGE_001, UNIT_DEFS, ENEMY_DEFS, createGreedyPolicy())
    const b = runHeadless(STAGE_001, UNIT_DEFS, ENEMY_DEFS, createGreedyPolicy())
