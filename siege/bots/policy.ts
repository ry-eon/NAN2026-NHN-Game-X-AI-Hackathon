...
export const greedy: BotPolicy = {
  ...
  act(s) {
    ...
    // 5) 마법사 — **성벽 보도 위**를 따라 움직여 궁극 사거리(14) 안에 전선을 넣는다.
    //    사거리 18→14 개정(2026-08-09) 후 6시드 스윕으로 두 번 재조정했다:
    //    ① 안뜰 지상(-13)은 벽까지 10을 버려 커버가 얇았다 — 기준선 이상 5/6·평균 1215
    //    ② **성벽 위**(x = east-3 안쪽 차선, h=11)는 6/6·평균 1420. 마법사는 원래
    //       성벽에서 시전하는 병종이라는 디렉션(사용자)과 수치가 같은 결론을 냈다.
    //    x를 보도 중앙선(병기가 늘어선 east)이 아니라 **안쪽 차선**으로 잡는 이유:
    //    고정 병기는 밀리지 않아 포좌 사이에 끼면 빠져나오지 못한다(기존 실측).
    if (!mage || s.tick % sec(4) !== 0) return undefined
    if (Math.abs(mage.pos.z - tz) <= 10 && mage.h > 5) return undefined
    return { unitMove: { ids: [mage.id], to: { x: CASTLE.east - 3, z: clampWallZ(tz), h: CASTLE.wallH } } }
  },
}
