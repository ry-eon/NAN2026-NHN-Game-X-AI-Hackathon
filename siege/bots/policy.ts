...
export const greedy: BotPolicy = {
  ...
  act(s) {
    ...
    //    사거리 18→14 개정(2026-08-09) 후 6시드 스윕으로 재조정. 결정적 노브는 문턱이
    //    아니라 **주기**였다(6→3초): 사거리가 짧으면 전선을 늦게 따라가는 대가가 곧
    //    '못 쏘는 시간'이다.
    //    **서는 자리 x=-13은 계단 띠(-12.4~-9.8)를 피한 값이다** — 구 -12/-11은 계단
    //    경사로 위라 마법사가 벽에 낀 것처럼 서 있었다(사용자 실측 반려 2026-08-09).
    if (!mage || s.tick % sec(3) !== 0) return undefined
    if (Math.abs(mage.pos.z - tz) <= 10) return undefined
    return { unitMove: { ids: [mage.id], to: { x: WALL_X - 7, z: clampWallZ(tz) } } }
  },
}
